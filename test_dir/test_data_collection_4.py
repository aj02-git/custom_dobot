...
from record_inv import RecordData
...
def recorder_worker(queue, record_obj):
    ...
    while True:
        try:
            ...
            timestamp, top_frame, wrist_frame, obs_pose, obs_angles, obs_gripper, actions_p, action_gripper, action_a = data_packet
            ...
            record_obj.collect_data_point(timestamp, top_frame, wrist_frame, obs_pose, obs_angles, obs_gripper,
                                          actions_p, action_gripper, action_a)
...
base_path = "dobot_data/23_June_pick_place_colored_boxes/obs_data"
...
while running:
    ...
    # timer_ = time.perf_counter()
    # print(" data", timer_ - timer)
    ...
    # _timer_ = time.perf_counter()
    # print("Before inverse", _timer_ - timer)
    action_a = r_obj.get_action_angles(actions_p)
    ...
    try:
        ...
        timer_ = time.perf_counter()
        print("Before Send Action", timer_ - timer)
        r_obj.send_angles(action_a)
        timer = time.perf_counter()
    ...
    data_packet = (timestamp, top_frame, wrist_frame, obs_pose, obs_angles, obs_gripper, actions_p, action_gripper, action_a)
    ...
    if sleep_duration > 0:
        print("sleep timer",sleep_duration)
